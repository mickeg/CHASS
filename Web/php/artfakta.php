...
239091
    ...
    //Denna listan är med demo-arterna:
    //$listoftaxons = [6010265, 216215, 100146, 100147, 212363, 1021469, 100165, 101535, 100270];
     //$listoftaxons = [100146]; 
    run($listoftaxons);
    ...
    function downloadImage($taxon){
        ...
        $html = file_get_contents($url, null, null);
        $MAX_FILES = 5;
    ...
    function getTaxonDescription($t){
        ...
        $query_name = "//h2[@class='taxon-title']/strong";
        $entries_name = $xpath->query($query_name);
        $r_name = $entries_name->item(0)->textContent;
        $r_utf8_name = utf8_decode($r_name);
        $result_name = trim(preg_replace('/\s+/', ' ', $r_utf8_name)); 

        $query_latin = "//h2[@class='taxon-title']/em";
        $entries_latin = $xpath->query($query_latin);
        $r_latin = $entries_latin->item(0)->textContent;
        $r_utf8_latin = utf8_decode($r_latin);
        $result_latin = trim(preg_replace('/\s+/', ' ', $r_utf8_latin)); 

        echo $result_latin;
        return unicodeString($t) .';'. unicodeString($result) .';'. $result_name. ';'. unicodeString($result_latin);
    }

    function getTaxonName($t){
        $GLOBALS['doc']->loadHTMLFile('http://artfakta.artdatabanken.se/taxon/'.$t);
        $xpath = new DOMXPath($GLOBALS['doc']);
        $query = "//h2[@class='taxon-title']/strong";
        $entries = $xpath->query($query);
        $r = $entries->item(0)->textContent;
        $r_utf8 = utf8_decode($r);
        $result = trim(preg_replace('/\s+/', ' ', $r_utf8)); 
    ...
    function createFile(){
        $file = 'artfakta.txt';
        $fp = fopen($file, 'w');
        fwrite($fp, 'TaxonID;Kännetecken;Namn;Latin'.PHP_EOL); 
    }
